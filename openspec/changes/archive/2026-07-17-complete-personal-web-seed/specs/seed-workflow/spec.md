...
- **WHEN** 开发者首次克隆仓库
- **THEN** START 说明 Node 22、安装、环境变量、开发命令、本地模式和质量门禁

#### Scenario: 配置外部增强

- **WHEN** 开发者需要云同步或 OpenAI 示例
- **THEN** START 说明 Supabase migration、RLS、Auth URL、公开变量、Magic Link、Key 保存与安全禁区
...
- **WHEN** tasks 全部完成
- **THEN** 新主规范严格校验通过，typecheck、lint、test、build 成功，README/START 与实现一致

### Requirement: 派生个人项目

START SHALL 列出从种子派生项目必须替换和验证的配置。
...
- 未使用 Supabase 或 OpenAI 的派生项目可以删除相应 UI，但不得破坏本地数据、恢复和质量门禁。
