...
interface UseCloudSyncOptions {
  repository: LocalDataRepository<TodoPayload>;
...
export const useCloudSync = ({
  ...
  const managerRef = useRef<SyncManager<TodoPayload> | null>(null);
  const authGatewayRef = useRef<CloudAuthGateway | null>(null);
  const operationRef = useRef<Promise<SyncResult<TodoPayload> | null> | null>(
    null,
  );
  ...
  const [conflict, setConflict] = useState<SyncConflict<TodoPayload> | null>(
    null,
  );
  ...
  const applyResult = useCallback(
    (result: SyncResult<TodoPayload>, successMessage: string) => {
  ...
  const execute = useCallback(
    async (
      action: (
        manager: SyncManager<TodoPayload>,
      ) => SyncResult<TodoPayload> | Promise<SyncResult<TodoPayload>>,
  ...
  useEffect(() => {
    ...
    void getSupabaseClient()
      .then((client) => {
        if (!active) return;
        const provider = new SupabaseSyncProvider<TodoPayload>({
        ...
        const manager = new SyncManager({
          repository,
          provider,
          isPayloadEmpty: (payload: TodoPayload) => payload.todos.length === 0,
        });
  ...
  useEffect(() => {
    const handleOnline = () => {
      if (autoSync && session && !conflict) void syncNow();
      else if (session) {
        setStatus("idle");
  ...
  }, [autoSync, conflict, session, syncNow]);
