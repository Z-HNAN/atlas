...
const createConflictScenario = async () => {
  ...
  a.update(() => ({ items: ["initial"] }));
  await managerA.sync();
  await managerB.sync();
  a.update(() => ({ items: ["from-a"] }));
  b.update(() => ({ items: ["from-b"] }));
  await managerA.sync();
...
describe("双设备同步生命周期", () => {
  it("第二台设备恢复后并发修改，选择本地可覆盖云端", async () => {
    ...
    expect(b.load().sync.dirty).toBe(false);
    expect(b.getLatestBackupJson()).not.toBeNull();
    expect(JSON.parse(b.getLatestRemoteBackupJson() ?? "{}")).toMatchObject({
  ...
  it("第二台设备并发修改时，选择云端会备份并覆盖本地", async () => {
    ...
    const resolved = managerB.resolveWithRemote();

    expect(resolved).toMatchObject({ status: "synced", action: "downloaded" });
    expect(b.load().payload.items).toEqual(["from-a"]);
    const backup = JSON.parse(b.getLatestBackupJson() ?? "{}") as {
      payload?: SyncPayload;
    };
